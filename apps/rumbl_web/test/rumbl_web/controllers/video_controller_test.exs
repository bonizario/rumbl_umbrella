defmodule RumblWeb.VideoControllerTest do
  ...
  @create_params %{url: "https://youtu.be", title: "Video", description: "Video Description"}
  @invalid_params %{title: "Invalid"}
  ...
  describe "with a logged-in user" do
    ...
    @tag login_as: "username"
    test "creates user video and redirects", %{conn: conn, user: user} do
      create_conn = post(conn, Routes.video_path(conn, :create), video: @create_params)
    ...
    @tag login_as: "username"
    test "does not create video, renders errors when given invalid params", %{conn: conn} do
      count_before = video_count()
      conn = post(conn, Routes.video_path(conn, :create), video: @invalid_params)
    ...
    test "lists all user's videos on index", %{conn: conn, user: user} do
      user_video = video_fixture(user, title: "Title")
      other_video = video_fixture(user_fixture(username: "other"), title: "Other Title")
  ...
  test "does not authorizes actions on data from other users", %{conn: conn} do
    owner = user_fixture(username: "owner")
    video = video_fixture(owner, @create_params)
    ...
    assert_error_sent :not_found, fn ->
      put(conn, Routes.video_path(conn, :update, video, video: @create_params))
    end
